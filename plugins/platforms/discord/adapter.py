...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def post_claude_bridge_decision(self, decision: Dict[str, Any]) -> None:
        """Post a claude_bridge escalation decision as a button prompt.

        See ``gateway.claude_bridge.OutboxWatcher`` for the outbox contract.
        """
        from plugins.platforms.discord import claude_bridge_ui
        await claude_bridge_ui.post_claude_bridge_decision(self, decision)
...
def _define_discord_view_classes() -> None:
    ...
    """
    global ExecApprovalView, SlashConfirmView, UpdatePromptView, ModelPickerView, ClarifyChoiceView, ChoicePickerView, DecisionOutboxView
    ...
    # Discord button View for a claude_bridge escalation decision; see
    # plugins.platforms.discord.claude_bridge_ui for the definition.
    from plugins.platforms.discord.claude_bridge_ui import define_decision_outbox_view
    DecisionOutboxView = define_decision_outbox_view()
if DISCORD_AVAILABLE:
    _define_discord_view_classes()
